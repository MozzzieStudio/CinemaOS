...
function App() {
  ...
  const handleScriptContextChange = useCallback(
  ...
  const handleAgentUpdateScript = useCallback(
  ...
  const dispatchCommand = (type: string, payload?: any) => {
    window.dispatchEvent(
      new CustomEvent("editor-command", { detail: { type, payload } })
    );
  };
  ...
  const menus = useMemo(
    () =>
      createMenus(dispatchCommand, {
        sceneNumbers: {
          enabled: sceneNumbersConfig.config.enabled,
          toggle: sceneNumbersConfig.toggleEnabled,
        },
        moresContinueds: {
          enabled: moresContinuedsConfig.config.enabled,
          toggle: moresContinuedsConfig.toggleEnabled,
        },
      }),
    [
      dispatchCommand,
      sceneNumbersConfig.config.enabled,
      moresContinuedsConfig.config.enabled,
    ]
  );
  ...
  const handleImportPDF = () => {
    alert(
      "PDF Import is currently experimental. Please use FDX for best results."
    );
  ...
  return (
    <div className="h-screen flex bg-[#0a0a0a] text-white overflow-hidden">
      ...
      {/* ═══════════ LEFT SIDEBAR ═══════════ */}
      <aside className="w-16 bg-[#0f0f0f] border-r border-white/6 flex flex-col items-center py-3 shrink-0">
        {/* Logo */}
        <div className="w-10 h-10 mb-6 bg-linear-to-br from-violet-500 to-purple-600 rounded-xl flex items-center justify-center font-bold text-lg shadow-lg shadow-violet-500/20 cursor-pointer hover:scale-105 transition-transform">
          C
        </div>
        ...
        <div className="flex flex-col gap-1">
          {/* User Avatar */}
          <div className="w-9 h-9 mx-auto mt-2 rounded-full bg-linear-to-br from-gray-600 to-gray-800 border-2 border-white/10 cursor-pointer hover:border-violet-500/50 transition-colors" />
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header
          ref={menuRef}
          className="h-9 bg-[#151515] border-b border-white/6 flex items-center px-2 shrink-0"
                  ...
                  <div className="absolute top-full left-0 mt-0.5 min-w-[220px] bg-[#1e1e1e]/98 backdrop-blur-xl border border-white/10 rounded-lg shadow-2xl py-1 z-50">
                    {menu.items.map((item: any, i) =>
                      item.divider ? (
                        <div
                          key={i}
                          className="h-px bg-white/8 my-1 mx-2"
                        />
                      ) : (
                        <button
                          key={i}
                          ...
                          className="w-full flex items-center justify-between px-3 py-1.5 text-[13px] text-white/70 hover:bg-white/8 hover:text-white transition-colors"
                        >
                          ...
                          {item.shortcut && (
                            <span className="text-white/30 text-[11px] ml-6">
                              {item.shortcut}
                            </span>
                          )}
                        </button>
        ...
        {/* CONTENT */}
        <main
          className="flex opacity-100 flex-1 min-h-0 relative"
          onClick={() => setActiveMenu(null)}
        >
          // Keep-Alive Pattern: // The ScriptEditor stays mounted (hidden) to
          preserve Lexical state/history. // Other heavy views (Canvas/PreProd)
          are unmounted to free resources.
          <div className="flex-1 flex flex-col min-h-0 relative">
            <div
              className="flex-1 flex flex-col min-h-0"
              style={{ display: viewMode === "writer" ? "flex" : "none" }}
            >
              <ScriptEditor
            ...
            {viewMode === "preproduction" && (
              <div className="flex-1 flex flex-col min-h-0">
                <PreProductionLayout projectId={projectId} />
              </div>
            )}
          ...
          {viewMode === "writer" && (
            <>
              ...
              />
              <WritingStatsPanel
                isOpen={isWritingStatsOpen}
                onClose={() => setWritingStatsOpen(false)}
              />
            </>
          )}
          {/* AI Agent Sidebar */}
          {isSidebarOpen && (
            <div className="absolute right-0 top-0 bottom-0 z-40 w-full sm:w-[380px] md:static md:w-[380px] md:shrink-0 border-l border-white/6 bg-[#0a0a0a] animate-in slide-in-from-right duration-200 shadow-2xl md:shadow-none">
              <AgentChatPanel
